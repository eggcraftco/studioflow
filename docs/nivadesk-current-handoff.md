...
| Side branches, all pushed, none merged | `ebay-workspace-allowlist` @ `120615ad` (**merged** into the deploy branch by `def97f49` — keep for history only); `activation-v21-cutover` @ `3f20e04f`; `retention-wiring` @ `55f6fa4d`; `onboarding-retention` @ `4d525de4` (the older source of the activation work) |
| Worktrees | `~/Developer/studioflow-retention` (branch `retention-wiring`), `~/Developer/studioflow-onboarding` (`onboarding-retention`), `~/Developer/studioflow-ebay-carry` (`ebay-carry-candidate`, superseded), publish repo `~/Developer/studioflow-hostinger-publish-20260530` (Round 170 `4e3a05f7` live) |
| Background jobs | none running; no pending deploy; no scheduled watches |
| Credentials | gcloud user + ADC + Firebase CLI were re-authenticated 10 Sep ~11:15Z after a session-reauth wall broke a deploy mid-run (check `gcloud auth print-access-token` before any long deploy) |
...
| Sandbox seller | **`TESTUSER_nivadesk_seller1`** (eBay sandbox user registered by the operator 10 Sep ~18:50Z; seller id `mtm4ubrcsv2`, site EBAY_GB); password known only to the operator |
...
| Not done / not approved | order, payment, refund, shipping, sync and import tests — **no approval given yet** for the next stage (it needs the twelve deployed by name with the switch, after a scope decision) |
...
| Rollback pointers | rollout §7 (step 1 resources), backend §6 (the sixteen), opening §… : delete the connection via `disconnectEbay`/document, remove the workspace entry from `appConfig/commerce`, revert `def97f49` with `git revert -m 1` if the gate must go |

## 2. Other live threads

| Thread | Verified state | Next |
|---|---|---|
| OpenAI ChatGPT app 1.2.0 | **In Review** (last read 13:3xZ on platform.openai.com/plugins: 1.2.0 Review, 1.1.1 Rejected, 1.0.0 Published). MCP/OAuth surface frozen: no redeploy of `chatgptMcp`, `chatgptOAuthAuthorize` or the other OAuth functions (revisions `chatgptmcp-00073-fuz`, `chatgptoauthauthorize-00045-has` unchanged); the review workspace `KSQidetb3oOSItE9amLISf9Lh6h2` / `review@nivadesk.app` untouched | wait for OpenAI; after a decision see memory `openai-review-next-steps` |
...
3. Take the operator's next instruction; the first pending decision is the eBay next stage (sync/import/order tests),
   which requires deploying the twelve switched-off functions by name and accepting that `autoSync` then runs.
