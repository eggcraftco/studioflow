...
| Review | Two adversarial reviewers raised sixteen findings, four of them high; all fixed on the branch. **A third pass then found that §5.5's central control — the presented-code registry — was described everywhere in the present tense and had never been built.** It is built now, with the rules block, the counters and the tests that go with it, and the sentence in section 2 below that depended on it has been rewritten. That pass also found two anonymous routes that could be flooded into refusing genuine sellers, a screen that could print a raw error code during this plan's own deploy window, a native start link that lost its state at the sign-in redirect, and a runbook (deploy plan §4.2) still describing the world before the browser-binding ticket. **A fourth pass then found that the callback page signed a fresh message for every presentation of the same ticket** — the one path with no rate limit of its own, so a copied cookie pair bought an unlimited number of signatures we mint and function invocations we pay for. The page now spends the ticket before it signs: the first landing connects, and every later one gets the same weaker message a stranger gets. The seller sees exactly the sentence they saw before. All fixed, each with a test that fails without it |
| Verification | Unit suite 1,310 passing and 0 failing, exit 0. The two eBay callback scripts — which compile the real web routes and drive them into the real Cloud Function — exit 0: `test:relay` (the transport, the browser-binding ticket, and the committed vectors) and `test:ebay-regressions` (the ten cases the review cites by id). The emulator chain (rules plus the end-to-end suites) exits 0. Web typechecks and builds. Mac and Android compile |
