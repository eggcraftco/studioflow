...
5. **Native workspace resolution falls back to the personal workspace on any read error and persists it.** `AuthViewModel.resolveActiveCompany` → `validateCompanyAccess` treats *any* `companies/<id>` read failure as "no access" and `activateCompany` then writes `users/<uid>.activeCompanyId = uid`. Seen twice with emulator token launches (Mac 14:28Z for the connected owner, iOS 14:42Z for the allowed owner): the user landed in a fresh personal workspace with the first-run wizard, and the real workspace id had been overwritten. Restored by hand in the emulator (`activeCompanyId` back to the workspace; `memberUids` added to `companies/qa-ebay-connected`). A transient permission or network error at launch could do the same to a real user; worth a separate item (fail closed to "keep the stored id, show the loading state", not "switch and persist").
...
13:35 emulator seed (three eBay workspaces, owner/member users) · 13:4x server matrix · 14:0x–14:2x Android app + Android Chrome flows · 14:2x–15:0x iOS app flows (QA account) and iPhone Safari (allowed) · 15:1x Mac denied + allowed · 15:2x–15:5x Mac connected (blocked twice by the persisted-fallback workspace, §4.5, then verified), iPhone Safari denied, iOS denied/allowed/connected · 16:0x record written.
