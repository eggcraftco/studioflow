...
scan, automatic agent updates, FileVault, firewall, a 15-minute screen lock (the
DPP 1.1.2 maximum) and automatic OS updates enforced by Intune; the device
reports Compliant. The Mac Studio
...
## The other Data Protection Policy controls — application answers

Full section-by-section mapping, statuses and the dated gap list:
`amazon-dpp-compliance-matrix.md`. The answers below are written the way the
application form asks them; every sentence points at a file in the pack.

- **Endpoint protection (DPP 1.1.2).** The single privileged endpoint is
  Intune-managed with Defender for Endpoint (tamper protection the user cannot
  disable), FileVault, firewall, automatic updates, and a screen lock after
  15 minutes of inactivity — the DPP maximum. Removable-media restriction is the
  one open item (gap G1). Evidence: `edr-onboarding-2026-09-05.md`, `device-inventory.md`.
- **Access management and least privilege (1.2–1.3).** One human identity;
  each workload runs as its own service account limited to its one job; access
  is granted by invitation, removed immediately, and reviewed on a recorded
  cadence (moving from six-monthly to quarterly, gap G2). Evidence:
  `access-control-policy.md`, `deploy-identity-2026-09-05.md`, `secret-iam.txt`.
- **Password and MFA (1.4.1).** MFA on every privileged account (passkeys,
  Authenticator, TOTP); 12-character, mixed-class passwords rotated annually,
  audited account by account on 4 Sep 2026 — length and class are attested, the
  providers do not display them. Ten-password history is being added to the
  policy (gap G4). Evidence: `password-and-mfa-policy.md` §6, §8.
- **Programmatic credentials (1.4.2).** The LWA client secret and each seller's
  refresh token live only in Secret Manager (encrypted at rest, europe-west2);
  no service-account keys can exist; rotation dates are set when Amazon issues
  the credentials (gap G5).
- **Encryption in transit and at rest (1.5, 2.4).** TLS everywhere;
  Google-managed AES-256 at rest on Firestore, Storage and Secret Manager with
  Google's key lifecycle management.
- **Risk management and incident response (1.6).** Incident response plan with
  roles, severity, clocks, playbooks, six-monthly review and tabletop; Amazon is
  notified within 24 hours at security@amazon.com; the incident management
  point of contact is named. A one-page annual risk register is being added
  (gap G6). Evidence: `incident-response-plan.md`.
- **Data deletion, retention and attribution (1.7, 1.8, 2.1).** Buyer PII is
  diverted to a server-only collection and deleted 30 days after fulfilment;
  Amazon Information lives in its own project and database; disconnecting a
  seller deletes the token. Evidence: `privacy/retention.js`,
  `access-control-policy.md` §5.
- **Data governance (2.2).** Privacy policy, account deletion and export exist
  for the product; a records-of-processing entry and DSAR path specific to
  Amazon Information are being added (gap G7).
- **Asset management and change management (2.3).** Infrastructure is scripted
  and read back by `verify-project.sh`; the evidence manifest is regenerated by
  script; the device inventory is re-confirmed quarterly; every production
  change is a commit and Amazon-zone deploys go through a logged deploy
  identity. Single-operator segregation is documented as a limitation with
  compensating controls.
- **Secure coding (2.5).** No credentials in code; emulator-based test
  environments separate from production; CI runs unit, rules and end-to-end
  suites on every push. GitHub secret scanning / push protection is gap G13.
- **Logging and monitoring (2.6).** Audit, data-access, network, DNS and WAF
  logs retained 400 days in a regional bucket; Security Command Center findings
  are delivered to the operator by e-mail (proven with real findings); a
  fortnightly log-review record is being started (gap G8).
- **Vulnerability management (2.7).** Written plan; first dependency scan
  recorded 5 Sep 2026 with fixes available for every finding and DPP deadlines
  adopted (critical 7 days, high 30 days); container-image scanning and a CI
  audit step planned; annual penetration test to be booked before resubmission
  (gap G9). Evidence: `vulnerability-management.md`, `vuln-scan-2026-09-05.md`.
- **Disaster recovery (2.7.4).** Main product: point-in-time recovery plus
  daily backups; Amazon zone: backups to be enabled after the freeze and the
  backup-region decision recorded (gap G10).
- **Subcontractors (2.8).** Sub-processors listed; none receives Amazon
  Information except Google as the infrastructure; a dated annual assessment
  note per sub-processor is being added (gap G14).
...
| B2 | **Cloud Run Threat Detection** produces no finding for Google's documented test; escalated by Google to their Product Specialist team for backend investigation (control 3 stays In Progress). **Event Threat Detection is no longer part of this blocker** — Google confirmed on 8 Sep 2026 that Malware: Bad Domain is anchored at organisation level and a project-only Premium deployment under a Standard-tier organisation cannot expose it, with no detector or configuration issue indicated | Google's Product Specialist team answers case **75151719** (next committed update 2026-09-10 22:30 IST / 17:00 UTC), either with a finding or with a statement that the detector is unavailable or defective in this configuration |
