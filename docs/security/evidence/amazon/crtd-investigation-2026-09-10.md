...
Google's `crtd-test` command from the 7 Sep e-mail (image, `sh -c`, payload, job-name prefix), plus what the 10 Sep e-mail
...
| Payload executed (container log) | **2026-09-10T09:40:44.030644Z** — `base64: f0VMRgIB: No such file or directory` (665 s after start; no "maximum timeout" line) |
| Container exit / completion | `Container called exit(0).` 09:40:54.061416Z; execution completed **09:40:57.318314Z**, succeeded 1 / failed 0 — the payload ran 235 s inside the 900 s limit |
...
| Effective task timeout / max retries | **900 s / 0** (read from the execution spec; the 9 Sep `sleep 660` run had the 600 s / 3 defaults and never reached the payload) |
...
the job's container log for the payload line `base64: f0VMRgIB: No such file or directory` (and any "maximum timeout"
...
| Payload | ran at **09:40:44.030644Z**, 665 s after start, 235 s inside the 900 s limit (container log line above) |
| Findings polls | **9 valid polls, HTTP 200 each**, 09:43:28Z → 10:23:50Z (every ~5 min): every poll returned the same two pre-existing findings — *Increasing Deny Ratio* (Cloud Armor, eventTime 06:35Z today, the load balancer backend) and *OS_LOGIN_DISABLED* (5 Sep) — and **no finding with an eventTime at or after the execution, none in any threat / execution / ELF / base64 / container category** (`findings-2.json` … `findings-10.json`) |
| `resource.type="threat_detector"` log entries since 09:29Z | none |
| Observation after the payload | 43 minutes (09:40:44Z → 10:23:50Z), longer than the 40-minute window used on 7 and 9 September |

**Reading.** This is the first `sleep 660` run in which the payload demonstrably executed inside the task's lifetime,
so the specialist's timing explanation for the 9 Sep failure is now excluded as a cause: with the timeout at 900 s the
payload ran, the task ended normally, and still no Cloud Run Threat Detection finding was created. The two unrelated
findings on every poll show the query reached Security Command Center and read this project correctly. Recorded as
**"detection not confirmed"**, not as "the detector works": a job that finishes is not evidence of detection, and no
finding can be tied to execution `…-8fmzk`.
...
control open, send the reply draft, and record the control as "not obtainable in this configuration" only if Google says
so in writing.
