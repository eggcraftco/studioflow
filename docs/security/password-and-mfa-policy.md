...
|---|---|---|---|---|
| 4 September 2026 | Gunes Gocmen | **Confirmed and corrected** — was Notify / no requirements / minimum 6; now Require / numeric / minimum 8 | **PASSED** — every privileged human account carries a second factor and a password rotated on 4 Sep 2026; length and character class are attested, which is the only evidence any of these services can produce | Initial version; Android brought into line with the other three clients; §6 given explicit length, character and rotation requirements; server-side policy enabled |
| *4 March 2027 (due)* | | | | |
...
control. This audit was completed on 4 September 2026 and every row closed. NivaDesk
therefore answers **Yes** to a questionnaire asking whether it enforces a
12-character, special-character, MFA-protected, annually-rotated password
policy for privileged accounts — on the evidence in the table below, and with
the length and character-class cells understood as attestations, because no
service displays either. The answer reverts to **No** the moment any row stops
being true. The document
...
|---|---|---|---|---|---|---|---|---|---|
| 1 | `contact@eggcraft.co.uk` | Google / Firebase / GCP | Owner on `eggcraft-studio` + 2 more roles | **Yes** — 2SV since 3 Jun, 2 passkeys, recovery phone and email set | **Attested** — generated in a password manager to the §6 standard on 4 Sep 2026; Google never displays length | **Attested** — same, and not displayable | **Yes — changed 4 September 2026 17:34**, confirmed in the account's own security event log (was 934 days) | **Attested** — first rotation done; the next is due 4 Sep 2027 and is recorded in §7 | **Closed** — length and character class are attestations, not readings; see the note below |
| 2 | `contact@eggcraft.co.uk` (billing name: Ecem Okumus) | Hostinger | Production web hosting; DNS for `nivadesk.co.uk` and `eggcraft.co.uk` | **Yes** — enabled 4 Sep 2026, reads back as "Etkin" | **Attested** — generated in a password manager to the §6 standard on 4 Sep 2026; Hostinger never displays length | **Attested** — same, and not displayable | **Yes — password set 4 Sep 2026** (the account had none before) | **Attested** — first rotation is this one; next due 4 Sep 2027 | **Closed** — rebuilt on 4 Sep 2026: account email moved from `eggcraftco@gmail.com` to the company address, a password of its own set, a recovery email added, the Google login disconnected. Length and character class are attestations, the same limit as row 1 |
| 3 | `eggcraftco` | GitHub | Admin on `eggcraftco/studioflow` (the production publish repo); sole collaborator; no organisation | **Yes** — enabled 4 Sep 2026: authenticator app Configured and set as the preferred method, plus a registered passkey. The security log shows both `user.two_factor_challenge_success` events, one solved with the authenticator app and one with the passkey | **Attested** — generated in a password manager to the §6 standard; GitHub never displays length | **Attested** — same | **Yes — reset 4 September 2026**, and unusually this one is *evidenced* rather than attested: the account's own security log carries `user.forgot_password` ("delivered to contact@eggcraft.co.uk") followed by `user.reset_password` | **Attested** — first rotation is this one | **Closed** |
...
| 5 | ~~The second GCP human account~~ | Google Cloud IAM | **There is no second human.** Read on 4 Sep 2026 from Cloud IAM with "Include Google-provided role grants" confirmed ON (24 principals listed): exactly one human, `contact@eggcraft.co.uk`. The organisation `eggcraft.co.uk` (378239481010) was read separately and holds the same single human | n/a | n/a | n/a | n/a | n/a | **Closed** — the finding does not exist. Firebase's "1 additional user has access to Firebase and/or Google Cloud resources" counts a non-human principal; the project holds one human and 23 service accounts and service agents |
| 6 | `gunes.gocmen@gmail.com` | Google | **None found.** Refused by the Firebase project; not the GitHub account (`eggcraftco`); not the Hostinger account (`eggcraftco@gmail.com`) | Yes — 2SV since 18 Apr 2023, 1 passkey | n/a | n/a | n/a | n/a | **Out of scope** — not privileged; revisit only if it turns out to hold Cloudflare access, which is still unverified |
...
- Cloudflare's and GitHub's passwords both had to be reset rather than merely
  accepted, because the account holder had never used either: sign-in was
  always through Google, so each password existed, was unknown, and could not
  be attested to any standard. An unknown password is not a verified one.
- GitHub is the one account whose password age is *evidenced* rather than
  attested. Its security log records the reset — request delivered to
  `contact@eggcraft.co.uk`, then the reset itself — which is a stronger
  artefact than anything the other services expose, and it is cited in the
  cell rather than summarised here.
- Row 5 is the finding that opened this audit, and it dissolved. The first read
  of Cloud IAM showed one human — but the "Include Google-provided role grants"
  filter was still on, and a filtered list is not evidence of absence. The
  second read confirmed the checkbox was off, turned it on, and produced 24
  principals: still one human. Both project and organisation were read. Firebase
  counts differently from IAM, and IAM is the authoritative source.
- Secret Manager access is not granted at project level to anybody — the roles
  are attached per secret, to the default compute service account, which is the
  separate remediation recorded in the access control policy. No human holds a
  Secret Manager role beyond what Owner implies.
