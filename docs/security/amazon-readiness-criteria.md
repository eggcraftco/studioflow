...
**Where this stands, 8 September 2026, after two first-hand responses from Google
Cloud Support on case 75151719.** Still **In Progress**, and deliberately not
Passed: the CRTD question is open inside Google. But four things are now settled,
three of them by Google rather than by us.

| | State |
|---|---|
| SCC pipeline, service agents, Pub/Sub delivery | **Confirmed healthy by Google.** Corroborated independently by a real finding (Cloud Armor "Increasing Deny Ratio") arriving end to end |
| `FAILED_PRECONDITION` / "project is already onboarded" | **Confirmed by Google to have no residual effect.** It meant the APIs were enabled before the console flow, nothing more |
| **ETD — Malware: Bad Domain** | **Closed. Not observable in the current SCC tier topology, confirmed by Google Support.** The finding loses its project metadata — Google's own known issue — and is anchored at organisation level; our organisation is Standard, so it has no entitlement to display a Premium ETD finding and the finding is dropped. A project-only Premium deployment inside a Standard-tier organisation *cannot* expose it. **No detector or configuration issue is indicated, and no further Bad Domain testing is to be performed** |
| **CRTD — Base64 ELF File Command Line** | **Formally escalated to Google's Product Specialist team for backend investigation** — whether the watcher attached to our gen-2 execution, and why the documented payload did not trigger. Google agreed our testing eliminated propagation time, regional support, enablement state and the delivery chain. Next status update committed for **2026-09-10 22:30 IST (17:00 UTC)** |

The distinction the ETD row draws is the one to carry into any submission: **not
observable** is not the same as **failed** or **misconfigured**. Nothing built
here is wrong, and nothing changeable inside the project would alter the result —
surfacing that finding would require Premium at the organisation level, which is
a purchasing decision rather than a remediation.

**This control passes when the CRTD question is answered**, either by a finding
appearing or by Google stating the detector is unavailable or defective in this
configuration. Until then the test job `amazon-crtd-verify-0907` stays in place
for their investigation and no further CRTD test is run unless Google asks.
