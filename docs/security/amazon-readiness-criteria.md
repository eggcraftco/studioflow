...
| 3. IDS / IPS / threat detection | **In progress — detectors on, detection not yet demonstrated** | Premium (30-day trial, then pay-as-you-go) on the project, Standard on the organisation; Event Threat Detection and Cloud Run Threat Detection effective (`scc-services.txt`); service agents bound (`scc-agents.txt`); findings → Pub/Sub → email alert built and **proven with a real finding** (Cloud Armor "Increasing Deny Ratio", 05:29 UTC: published 3 s after creation, held by the evidence subscription, alert fired — `scc-finding-2026-09-05.md`); 400-day log retention; Cloud Armor prevention half live | Google's documented tests were run once each on 5 Sep — ETD "Malware: Bad Domain" (Cloud Run resolver, then the official VM procedure with the query logged under the VM's name) and CRTD "Base64 ELF File Command Line" (job ran the exact command) — and **no finding appeared** in the project or the organisation within Google's stated latency (`scc-test-2026-09-05.txt`, `scc-crtd-test-2026-09-05.txt`). Not re-triggered. Google documents CRTD activation as "up to 3.5 hours for newly onboarded projects" — the 03:01 test ran inside that window and must be repeated once after it (morning decision). ETD activation is "within seconds" with detection "under 15 minutes", so its silence is structural; leading hypothesis: the project's `_Default` log sink is disabled by design and ETD may depend on it (test = re-enable it temporarily and run the VM procedure once — morning decision). `Passed` only when both findings and their delivery are recorded |
