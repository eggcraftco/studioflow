...
check("eleven Square functions are exported and the queue dispatcher routes provider square to the connector", () => {
  for (const name of ["beginSquareConnect", "squareOAuthCallback", "getSquareConnections", "updateSquareConnectionSettings", "disconnectSquare", "squareWebhook", "reconcileSquareConnections", "syncSquareNow", "previewSquareImport", "runSquareImport", "listSquareUnmatched"]) assert.ok(index.includes(`exports.${name} = squareExports.${name};`), name);
...
check("the connector goes through the engine, never writes an order from a payment, and boxes tokens (SQ-ORD-011, SQ-PAY-006, SQ-AUTH-005)", () => {
  ...
  assert.ok(connector.includes('cursors.readCursor(db(), "square", ref.id, "order")') && connector.includes('cursors.readCursor(db(), "square", ref.id, "payment")') && connector.includes('cursors.readCursor(db(), "square", ref.id, "events")'), "SQ-REC-009: separate cursors");
});
