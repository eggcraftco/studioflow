...
const real = (id, extra = {}) => ({ id, companyId: "c1", customerName: "Ada Lovelace", orderValue: 120, paymentDate: "2026-09-10T10:00:00.000Z", ...extra });
...
(async () => {
  ...
  await check("the first substantive order makes the prompt eligible once per seven days; the showing is recorded as an event", async () => {
    ...
    assert.strictEqual(other.show, true); assert.ok(store.read("companies/c1/feedbackState/u2"));
  });
  ...
  await check("the pure eligibility rule, on its own: no clock or no first success is a no; the caps come from messaging.js", async () => {
    const now = Date.parse("2026-09-10T12:00:00.000Z");
    assert.strictEqual(feedback.promptEligibility({ enabled: true, firstOrder: { state: "substantive" } }).reason, "no_clock");
    assert.strictEqual(feedback.promptEligibility({ nowMs: now, enabled: true, firstOrder: { state: "none" } }).reason, "no_first_success");
    assert.strictEqual(feedback.promptEligibility({ nowMs: now, enabled: true, firstOrder: { state: "substantive" }, state: {} }).show, true);
    assert.strictEqual(feedback.promptEligibility({ nowMs: now, enabled: true, firstOrder: { state: "substantive" }, state: { shows: [{ campaign: feedback.CAMPAIGN_FIRST_SUCCESS, atMs: now - DAY }] } }).reason, "already_sent");
    assert.strictEqual(feedback.promptEligibility({ nowMs: now, enabled: true, firstOrder: { state: "substantive" }, state: { shows: [{ campaign: feedback.CAMPAIGN_FIRST_SUCCESS, atMs: now - 8 * DAY }] } }).show, true);
    assert.strictEqual(feedback.promptEligibility({ nowMs: now, enabled: true, firstOrder: { state: "substantive" }, state: { dismissals: [{ campaign: feedback.CAMPAIGN_FIRST_SUCCESS, atMs: now - 29 * DAY }] } }).reason, "dismissed_recently");
    assert.strictEqual(feedback.promptEligibility({ nowMs: now, enabled: true, firstOrder: { state: "substantive" }, state: { dismissals: [{ campaign: feedback.CAMPAIGN_FIRST_SUCCESS, atMs: now - 31 * DAY }] } }).show, true);
