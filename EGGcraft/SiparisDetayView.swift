...
@AppStorage("pdfShowCustomer") private var pdfShowCustomer = true; @AppStorage("pdfShowContact") private var pdfShowContact = true; @AppStorage("pdfShowPreview") private var pdfShowPreview = true; @AppStorage("pdfShowFinCustomer") private var pdfShowFinCustomer = true; @AppStorage("pdfShowPaymentMethod") private var pdfShowPaymentMethod = true; @AppStorage("pdfShowFinInternal") private var pdfShowFinInternal = false; @AppStorage("pdfShowStatus") private var pdfShowStatus = true; @AppStorage("pdfShowShipping") private var pdfShowShipping = true; @AppStorage("pdfShowAddress") private var pdfShowAddress = true; @AppStorage("pdfShowShippingAddress") private var pdfShowShippingAddress = true
    ...
    private func phoneCompactSummaryText(for kart: KartTipi) -> String {
        ...
        case .financial:
            ...
            let refundedTail = (siparis.refundedAmount ?? 0) > 0
                ? " • " + t("Refunded", lang: seciliDil) + " " + privacyCurrency(siparis.refundedAmount ?? 0, symbol: seciliParaBirimi, ondalik: seciliOndalik, hideNumbers: hideSensitiveNumbers)
                : ""
            return t("Paid", lang: seciliDil) + " \(paid) • " + t("Remaining", lang: seciliDil) + " \(remaining)" + refundedTail
        case .status:
    ...
    private func deletePayment(_ entry: PaymentEntry) {
        // A refund or chargeback entry belongs to a bank row: taking it off the
        // order is the bank module's unlink (entry out, paid and refunded totals
        // restored, the row freed) — never a local rewrite of the ledger.
        if entry.refund == true, let transactionId = entry.bankTransactionId, !transactionId.isEmpty {
            Task {
                do {
                    _ = try await firebaseManager.bankRefundLink(transactionId: transactionId, mode: "unlink", orderId: "", kind: "")
                } catch {
                    print("Refund unlink failed: \(error)")
                }
            }
            return
        }
    ...
    private func financialKarti(colIndex: Int) -> some View {
        DetayKarti(
            ...
            if let refunded = siparis.refundedAmount, refunded > 0 {
                // Money handed back: the sum of the refund and chargeback entries in the ledger — kept by the server, read here, never typed.
                HStack {
                    Text(t("Refunded", lang: seciliDil)).font(.system(size: 13, weight: .semibold)).foregroundColor(.secondary)
                    Spacer()
                    Text(privacyCurrency(refunded, symbol: seciliParaBirimi, ondalik: seciliOndalik, hideNumbers: hideSensitiveNumbers))
                        .font(.system(size: 13, weight: .bold)).monospacedDigit().foregroundColor(.red)
                }
            }
