// Rebrands a raw Firebase Storage download URL as a branded viewer link for the
...
//
// When the workspace has a customer-link name (workspace.clientPortalHost —
// their own domain or their nivadesk.app subdomain), pass it as brandedHost and
// the link carries THEIR name: https://track.eggcraft.co.uk/f/…
...
function originFor(brandedHost?: string) {
  const host = (brandedHost || "").trim().toLowerCase().replace(/^https?:\/\//, "").replace(/\/.*$/, "");
  return host ? `https://${host}` : PUBLIC_ORIGIN;
}

// Creates a short, clean branded link (company id + token hidden) by storing
// a mapping server-side. Falls back to the path-based masked URL on any failure.
export async function createSharedFileLink(rawUrl: string | null | undefined, brandedHost?: string): Promise<string> {
  ...
  try {
    ...
    if (id) {
      ...
      return `${originFor(brandedHost)}/f/${id}${ext}`;
    }
  } catch {
    // ignore — fall back below
  }
  return maskFileUrl(rawUrl, brandedHost);
}
...
export async function openSharedFile(rawUrl: string | null | undefined, brandedHost?: string): Promise<void> {
  if (!rawUrl || typeof window === "undefined") return;
  const tab = window.open("about:blank", "_blank");
  let link = rawUrl;
  try {
    link = await createSharedFileLink(rawUrl, brandedHost);
  } catch {
    link = maskFileUrl(rawUrl, brandedHost);
  }
  if (tab && !tab.closed) {
...
export async function downloadSharedFile(rawUrl: string | null | undefined, fileName?: string, brandedHost?: string): Promise<void> {
  if (!rawUrl || typeof window === "undefined") return;
  try {
    ...
    // bar still shows the branded host instead of firebasestorage.
    await openSharedFile(rawUrl, brandedHost);
  }
}

export function maskFileUrl(rawUrl: string | null | undefined, brandedHost?: string): string {
  if (!rawUrl) return rawUrl ?? "";
  try {
    ...
    return `${originFor(brandedHost)}/f/${segments}?b=${encodeURIComponent(bucket)}&t=${encodeURIComponent(token)}`;
  } catch {
    return rawUrl;
  }
}
